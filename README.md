...
- **Keep important files out of harm's way.** The latest Xcode archive for each app, installed apps, and other personal storage stay in a separate Review Only lane. Older archive copies can be cleaned after confirmation.
...
caches, older Xcode archive copies, browser AI models, editor/updater caches,
and package caches. Stop active developer tools, review the displayed targets
and report-only guidance, then confirm once. The latest Xcode archive for each
app stays review-only.
...
Preview deleting older Xcode archives while keeping the latest archive for each
app:
...
clone is shut down. Archive cleanup also refuses to delete the newest archive
for each app.
...
- Older Xcode archive copies: `~/Library/Developer/Xcode/Archives/YYYY-MM-DD/*.xcarchive` (keeps the latest archive for each app)

Report-only locations:

- Latest Xcode archive per app: `~/Library/Developer/Xcode/Archives`
...
and other completed documents. Older Xcode archives can be cleaned in the app
while the latest archive for each app stays on Review Only. Project Shelf is deliberately limited to folders the user selects or
...
- `--xcode-caches` selects all supported Xcode, DeviceSupport, XCTest clone, and simulator cache categories. It does not delete Xcode archives.
- `--xcode-archive-copies` deletes older `.xcarchive` bundles and keeps the latest archive for each app name. Archives that only differ by spaces or punctuation, such as `BlackwingVault` and `Blackwing Vault`, are treated as the same app. iOS and tvOS archives stay separate even when they share a bundle identifier.
...
- The latest Xcode archive for each app, Docker data, and active-session runtime caches are report-only.
